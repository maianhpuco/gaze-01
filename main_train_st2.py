#!/usr/bin/env python3
"""
Weighted multi-label training for Silence-Thought with threshold tuning.

This script extends the baseline trainer by computing per-class positive weights
for BCE loss, reporting richer metrics (macro/micro F1 and PR-AUC), and tuning
per-class decision thresholds on the validation set to maximise F1.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train silence-thought RNN (pos-weighted) on EGD-CXR data")
