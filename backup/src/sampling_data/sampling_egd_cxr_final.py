#!/usr/bin/env python3
"""
Final EGD-CXR Dataset Sampling Script

This script creates a diverse and comprehensive sample from the EGD-CXR dataset,
ensuring representation across all clinical conditions with practical data validation.
...
class FinalEGDCXRSampler:
    """Final class to handle diverse sampling from the EGD-CXR dataset."""
    
    def __init__(self, config_path):
        """Initialize the final sampler with configuration."""
        ...
        logger.info(f"Initialized Final EGD-CXR Sampler")
    ...
    def create_diverse_sample(self, df, n_samples=50):
        ...
        # 1. Sample from primary conditions (Normal, CHF, Pneumonia) - 10 each = 30 total
        primary_samples_per_condition = 10
        ...
        # 2. Sample studies with multiple conditions (complex cases) - 8 total
        multi_condition_data = df[~df['dicom_id'].isin(used_ids)]
        ...
        # 3. Sample from secondary conditions - 3 each = 12 total
        secondary_samples_per_condition = 3
        ...
        if samples:
            ...
            final_sample = df.sample(n=min(n_samples, len(df)), random_state=42)
    ...
    def create_comprehensive_metadata(self, sample_df, gaze_records, fixation_records, bbox_records, audio_count):
        ...
        metadata = {
            'sample_info': {
                ...
                'sampling_strategy': 'diverse_stratified_final'
            },
            'demographics': {
    ...
    def create_detailed_summary_report(self, sample_df, metadata):
        ...
        with open(report_file, 'w') as f:
            f.write("EGD-CXR Final Dataset Sampling Summary\n")
            f.write("=" * 60 + "\n\n")
    ...
    def run_final_sampling(self):
        """Run the complete final sampling process."""
        logger.info("Starting Final EGD-CXR dataset sampling...")
        ...
        try:
            ...
            logger.info("Final sampling completed successfully!")
            logger.info(f"Output directory: {self.output_path}")
            
            return sample_df, metadata
            
        except Exception as e:
            logger.error(f"Error during final sampling: {str(e)}")
            raise

def main():
    """Main function to run the final sampling process."""
    config_path = "/project/hnguyen2/mvu9/folder_04_ma/gaze-01/config_maui/data_egd-cxr.yaml"
    
    try:
        sampler = FinalEGDCXRSampler(config_path)
        sample_df, metadata = sampler.run_final_sampling()
        
        print("\n" + "="*70)
        print("FINAL EGD-CXR SAMPLING COMPLETED SUCCESSFULLY!")
        print("="*70)
