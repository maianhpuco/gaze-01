...
def encode_with_tau(transcripts, vocab, max_len, device):
    encoded = []
    for tr in transcripts:
        text = tr.get('text', '') if isinstance(tr, dict) else ''
        tokens = vocab.tokenize(text)
        ids = [vocab.word2idx.get(t, vocab.word2idx["<UNK>"]) for t in tokens]
        ids = [vocab.word2idx["<BOS>"]] + ids + [vocab.word2idx["<EOS>"]]
        ids = ids[:max_len] if len(ids) > max_len else ids + [vocab.word2idx["<PAD>"]] * (max_len - len(ids))
        encoded.append(torch.tensor(ids, dtype=torch.long, device=device))
    return torch.stack(encoded), None
...
class CXRDenseNet121(nn.Module):
    """TorchXRayVision DenseNet‑121, CheXpert/MIMIC pretrained → 1024‑d features."""
    def __init__(self):
        super().__init__()
...
class TMRNN(nn.Module):
    """
    Temporal Multimodal RNN for EGD‑CXR with multitask learning (classification + transcript).

    Encoder: GRU over s_t = concat(φ_g([x,y,Δt]), W_r·roi_hits, v_img_proj)
    Decoder: T5 (conditional generation) attending over projected GRU states.
    """
    def __init__(
        self,
        num_seg: int,
    ...
    def _delta_t(times: torch.Tensor) -> torch.Tensor:
        ...
        return torch.clamp(dt, min=0.0).unsqueeze(-1)
    ...
    def forward(
        ...
        images = batch["images"]
        fix = batch["fixations"]
        xy = fix["xy_norm"]                 # [B,T,2] normalized to [0,1]
        times = fix["time"]                  # [B,T]
        ...
        img_t = img_vec.unsqueeze(1).expand(B, T, -1)

        # s_t and RNN
        s_t = torch.cat([g, r, img_t], dim=-1)
