...
class TMRNN(nn.Module):
    ...
    def _build_backbone(self, name: str) -> Tuple[nn.Module, int]:
        ...
        if name == "txrv_densenet121":
            ...
            # Use gold-aligned weights and call .features(...) (not .forward)
    ...
    def forward(
        ...
        # ------- Text head (gold-aligned: bypass T5 encoder) -------
        ...
        if self.enable_text_head and transcripts_ids is not None and self.use_teacher_forcing:
            ...
            # Use BaseModelOutput so T5 can index encoder_outputs[0]
            enc_out = BaseModelOutput(last_hidden_state=enc_hidden)

            # Manual teacher forcing: feed decoder inputs (shifted right)
            dec_in = transcripts_ids[:, :-1]                             # [B,L-1]
            out = self.t5(
                encoder_outputs=enc_out,
                decoder_input_ids=dec_in,
                return_dict=True,
            )
            # Align with trainer's CE against transcripts_ids[:, 1:]
            txt_logits = out.logits                                      # [B,L-1,V]
