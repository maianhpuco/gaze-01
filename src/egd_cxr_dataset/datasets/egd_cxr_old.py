...
class ClassificationLabel:
    """
    Single-label classification payload (one-vs-rest) for CHF / pneumonia / Normal.
    """
...
class EGDCXRDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            classes: Ordered tuple of class names for single-label classification.
            drop_unlabelled: Whether to discard cases without a clear label.
        ...
        self.single_classes: Tuple[str, ...] = tuple(cls.strip() for cls in classes if cls and str(cls).strip())
        if not self.single_classes:
            raise ValueError("`classes` must contain at least one non-empty class name.")
        ...
        for case_id in base_cases:
            ...
            if row.empty:
                continue
            classification = self._classify_row(row.iloc[0])
            if classification.index < 0:
                if self.drop_unlabelled:
                    continue
            filtered_cases.append(case_id)
            classification_map[case_id] = classification
            targets.append(classification.index)

        if not filtered_cases:
            raise ValueError("No cases with valid single-label classification were found.")
    ...
    def sample_weights(self) -> torch.Tensor:
        ...
        targets = torch.tensor(self._single_targets, dtype=torch.long)
    ...
    def _classify_row(self, row: pd.Series) -> ClassificationLabel:
        ...
        for cls in self.single_classes:
            if per_class.get(cls, 0) == 1:
                index = self.single_classes.index(cls)
                name = cls
                break
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # Compute presence vectors for segments and bounding boxes
        if num_segments > 0:
        ...
        for box in boxes:
            if 0 <= box.cls_id < self.num_box_classes:
                box_presence[box.cls_id] = 1.0
...
def collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    segment_presence = torch.stack([item["segment_presence"] for item in batch], dim=0)
    box_presence = torch.stack([item["box_presence"] for item in batch], dim=0)
    ...
    classification_indices = torch.tensor(
        [item["labels"]["classification"]["index"] for item in batch],
        dtype=torch.long,
    )
    ...
    # Retain binary labels for reference (if needed downstream)
    labels_dict = {
    ...
    return {
        ...
        "segment_presence": segment_presence,  # (B, num_segments)
        "box_presence": box_presence,     # (B, num_box_classes)
