...
from torchvision import transforms  # Added for augmentation
...
class AugmentedEGDCXRDataset(EGDCXRDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform and 'images' in item:
            # Assume item["images"] is torch.Tensor [1, H, W] (grayscale)
            img = transforms.ToPILImage()(item["images"].squeeze(0))  # To PIL
            img = self.transform(img)
            item["images"] = transforms.ToTensor()(img).unsqueeze(0)  # Back to [1, H, W]
        return item
...
def main() -> None:
    ...
    parser.add_argument(
        "--image_backbone",
        type=str,
        default=None,  # if None, take from YAML options.image_backbone
    ...
    parser.add_argument("--unfreeze_after", type=int, default=5, help="Unfreeze backbone after this many epochs (0 to disable staged unfreezing)")
    ...
    test_ids = None
    if (split_dir / "test_ids.txt").exists():
    ...
    # Add augmentation only for train
    ...
    val_ds   = AugmentedEGDCXRDataset(transform=None, **common_kwargs, case_ids=val_ids)
    test_ds  = AugmentedEGDCXRDataset(transform=None, **common_kwargs, case_ids=test_ids) if test_ids is not None else None
    ...
    # --------------- Optimizer ------------
    opt = AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=float(cfg["train"]["lr"]), weight_decay=float(cfg["train"]["weight_decay"]))
    ...
    for ep in range(1, epochs + 1):
        ...
        if args.unfreeze_after > 0 and ep == args.unfreeze_after + 1:
            ...
            # Update optimizer to include new params (though not strictly needed, as params are now grad-enabled)
            opt = AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=opt.param_groups[0]['lr'], weight_decay=float(cfg["train"]["weight_decay"]))

        tr_loss, tr_acc = run_one_epoch(model, train_loader, cfg, device, optimizer=opt, train=True)
        va_loss, va_acc = run_one_epoch(model, val_loader,   cfg, device, optimizer=None, train=False)
    ...
    if test_loader is not None:
        te_loss, te_acc = run_one_epoch(model, test_loader, cfg, device, optimizer=None, train=False)
